...
#define _UNICODE
#include <Windows.h>
#include <commctrl.h>  // Para controles comunes
#include <stdio.h>     // Para swprintf
...
int WINAPI WinMain(
    ...
    // Inicializar controles comunes (IMPORTANTE)
...
void PerformOperation(int operation) {
    ...
    switch(operation) {
        ...
        case 3: // División
            if (num2 == 0) {
                wcscpy(result, L"Error: División por cero");
    ...
    // Usar swprintf en lugar de StringCchPrintfW
    swprintf(result, 100, L"%.2f%s%.2f = %.2f", num1, operationSymbol, num2, calculation);
    SetWindowTextW(hResult, result);
}